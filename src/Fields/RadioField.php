...
/**
 * Radio button group field wrapped in fieldset/legend for ADA compliance.
 */
class RadioField extends AbstractField {
	...
	/**
	 * Render the radio group HTML.
	 *
	 * Uses fieldset/legend per WCAG for grouped radio buttons.
	 *
	...
	public function render( array $field, mixed $value, array $context = [] ): string {
		...
		foreach ( $options as $index => $option ) {
			...
			$opt_id    = $prefix . '-' . $index;
			$checked   = checked( (string) $value, (string) $opt_value, false );

			$html .= '<div class="shqf-radio-item">';
			$html .= '<input type="radio" id="' . esc_attr( $opt_id ) . '" name="' . $name . '"';
