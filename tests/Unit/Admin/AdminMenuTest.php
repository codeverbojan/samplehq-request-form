...
class AdminMenuTest extends TestCase {
	...
	/**
	 * Connection callback success stores connection and shows success notice.
	 */
	public function test_connection_callback_success(): void {
		Monkey\Functions\stubs( [
			'sanitize_text_field' => static fn( $s ) => $s,
			'wp_unslash'          => static fn( $s ) => $s,
			'get_current_user_id' => static fn() => 1,
			'admin_url'           => static fn( $s ) => 'http://test/' . $s,
			'__'                  => static fn( $s ) => $s,
			'get_transient'       => static fn() => false,
		...
		$connection_data = [
			...
			'workspace_id'      => 42,
			'workspace_name'    => 'Acme',
			'connection_secret' => 'secret123',
			'connected_by'      => 'admin@acme.com',
		];

		$cm = \Mockery::mock( ConnectionManager::class );
		$cm->shouldReceive( 'validate_callback' )
			->once()
			->with(
				[
					'state'            => 'token123',
					'connection_token' => 'dGVzdA==',
					'signature'        => 'sig456',
				],
				1
			)
			->andReturn( $connection_data );

		$cm->shouldReceive( 'store_connection' )
			->once()
		...
		Monkey\Functions\expect( 'wp_safe_redirect' )
			->once()
			->with( \Mockery::pattern( '/tab=connection/' ) )
			->andReturnUsing( static function () {
				throw new \RuntimeException( 'redirected' );
			} );

		$_GET                       = [ 'page' => 'shqf-settings', 'action' => 'callback' ];
		...
		$_GET    = [];
		$_POST   = [];
		$_SERVER = array_diff_key( $_SERVER, [ 'REQUEST_METHOD' => 1 ] );
	}

	/**
	 * Connection callback validation failure shows error notice.
	 */
	public function test_connection_callback_validation_failure(): void {
		Monkey\Functions\stubs( [
			'sanitize_text_field' => static fn( $s ) => $s,
			'wp_unslash'          => static fn( $s ) => $s,
			'get_current_user_id' => static fn() => 1,
		...
		$cm->shouldReceive( 'validate_callback' )
			->once()
			->andReturn( new \WP_Error( 'shqf_state_mismatch', 'State mismatch.' ) );

		$cm->shouldNotReceive( 'store_connection' );
	...
	/**
	 * Connection callback non-POST request redirects without processing.
	 */
	public function test_connection_callback_non_post_redirects(): void {
		Monkey\Functions\stubs( [
			'sanitize_text_field' => static fn( $s ) => $s,
			'wp_unslash'          => static fn( $s ) => $s,
			'admin_url'           => static fn( $s ) => 'http://test/' . $s,
		...
		$cm = \Mockery::mock( ConnectionManager::class );
		$cm->shouldNotReceive( 'validate_callback' );
