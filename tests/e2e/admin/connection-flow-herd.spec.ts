...
function phpEval( wpRoot: string, php: string ): string {
	...
	const raw = execSync( `php -r '${ escaped }'`, {
		encoding: 'utf-8',
		timeout: 15000,
	} );
	return raw.trim();
...
function clearMailHog(): void {
	...
	if ( data.total > 0 ) {
		throw new Error(
			`MailHog clear failed: ${ data.total } messages remain`
		);
	}
}
...
async function getVerificationCode(
	...
	for ( let i = 0; i < retries; i++ ) {
		const raw = execSync(
			`curl -s "${ MAILHOG_API }/api/v2/search?kind=to&query=${ encodeURIComponent(
				email
			) }"`,
			{ encoding: 'utf-8', timeout: 5000 }
		);
	...
	throw new Error(
		`No verification code found for ${ email } after ${ retries } retries`
	);
}
...
test.describe( 'E2E-6: Full connection flow (Herd)', () => {
	...
	test.beforeAll( () => {
		...
		originalEmail = herdEval(
			'$u = get_user_by("ID", 1); echo $u->user_email;'
		);
		herdEval(
		...
		herdEval(
			'delete_option("shqf_connection"); delete_option("shqf_connect_state");'
		);
