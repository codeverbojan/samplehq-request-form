...
function wpEval( php: string ): string {
	...
	return execSync( `npx wp-env run cli -- wp eval '${ escaped }'`, {
		...
		.split( '\n' )
		.filter( ( l ) => ! l.startsWith( '\u001b' ) && ! l.includes( 'Starting' ) && ! l.includes( 'Ran `' ) && l.trim() !== '' )
		.join( '\n' )
		.trim();
}
...
test.describe( 'Form builder', () => {
	test( 'builder loads with 3 panels', async ( { page } ) => {
		...
		await expect( page.locator( '.shqf-builder-palette-item' ).first() ).toBeVisible();
	} );
	...
	test( 'click palette item adds field to canvas', async ( { page } ) => {
		...
		await page.locator( '.shqf-builder-palette-item', { hasText: 'Text' } ).first().click();
		await expect( page.locator( '.shqf-builder-field' ) ).toHaveCount( before + 1 );
		await expect( page.locator( '.shqf-builder-field-label' ).last() ).toContainText( 'Text' );
	} );
	...
	test( 'click field shows settings', async ( { page } ) => {
		...
		// Add a text field.
		await page.locator( '.shqf-builder-palette-item', { hasText: 'Text' } ).first().click();
		await expect( page.locator( '.shqf-builder-field' ).last() ).toBeVisible();
		// Click the field to select it.
		await page.locator( '.shqf-builder-field' ).last().click();
		// Settings panel should show field config.
		await expect( page.locator( '.shqf-builder-field--selected' ) ).toHaveCount( 1 );
		await expect( page.locator( '.shqf-builder-settings-header' ) ).toContainText( 'Text' );
	} );
	...
	test( 'change field label updates canvas', async ( { page } ) => {
		...
		const labelInput = page.locator( '.shqf-builder-settings .components-text-control__input' ).first();
		await labelInput.fill( 'Full Name' );
		// Canvas should update.
		await expect( page.locator( '.shqf-builder-field-label' ).last() ).toContainText( 'Full Name' );
	} );
	...
	test( 'toggle required shows asterisk', async ( { page } ) => {
		...
		await expect( page.locator( '.shqf-builder-field' ).last().locator( '.shqf-builder-required' ) ).toHaveCount( 0 );
		// Toggle required on.
		await page.locator( '.shqf-builder-settings .components-form-toggle__input' ).check();
		// Asterisk should appear.
		await expect( page.locator( '.shqf-builder-field' ).last().locator( '.shqf-builder-required' ) ).toBeVisible();
	} );
	...
	test( 'delete field from canvas', async ( { page } ) => {
		...
		await page.locator( '.shqf-builder-field-actions .is-destructive' ).last().click();
	...
	test( 'save form shows success notice', async ( { page } ) => {
		...
		await page.locator( '.shqf-builder-palette-item', { hasText: 'Email' } ).first().click();
		await page.waitForTimeout( 300 );
		// Click Save.
		await page.locator( '.shqf-builder-header-right' ).locator( 'button', { hasText: 'Save' } ).click();
		// Success notice should appear.
		await expect( page.locator( '.shqf-builder-notice--success' ) ).toBeVisible( { timeout: 5000 } );
	} );
	...
	test( 'undo/redo works', async ( { page } ) => {
		...
		const countBefore = await page.locator( '.shqf-builder-field' ).count();
		// Add a field.
		await page.locator( '.shqf-builder-palette-item', { hasText: 'Phone' } ).first().click();
		await expect( page.locator( '.shqf-builder-field' ) ).toHaveCount( countBefore + 1 );
		// Undo (Ctrl+Z).
		await page.keyboard.press( 'Control+z' );
		await page.waitForTimeout( 300 );
		await expect( page.locator( '.shqf-builder-field' ) ).toHaveCount( countBefore );
		// Redo (Ctrl+Shift+Z).
