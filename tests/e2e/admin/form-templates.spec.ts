...
test.describe( 'Form templates', () => {
	test( 'template selector shows 4 templates', async ( { page } ) => {
		...
		await expect( page.locator( '.shqf-template-title' ).nth( 0 ) ).toContainText( 'Wizard' );
		await expect( page.locator( '.shqf-template-title' ).nth( 1 ) ).toContainText( 'Grid' );
		await expect( page.locator( '.shqf-template-title' ).nth( 2 ) ).toContainText( 'Checklist' );
		await expect( page.locator( '.shqf-template-title' ).nth( 3 ) ).toContainText( 'Scratch' );
	} );

	test( 'create form from wizard template', async ( { page } ) => {
		await page.goto( TEMPLATES_URL );
		// Click "Use Template" on the wizard card.
		await page.locator( '.shqf-template-card' ).nth( 0 ).locator( '.shqf-template-btn' ).click();
	...
	test( 'create form from grid template', async ( { page } ) => {
		await page.goto( TEMPLATES_URL );
		await page.locator( '.shqf-template-card' ).nth( 1 ).locator( '.shqf-template-btn' ).click();
		await expect( page.locator( '.shqf-builder-palette' ) ).toBeVisible();
		const fields = await page.locator( '.shqf-builder-field, .shqf-builder-row' ).count();
	...
	test( 'create form from blank template', async ( { page } ) => {
		...
		await page.locator( '.shqf-template-card' ).nth( 3 ).locator( '.shqf-template-btn' ).click();
		await expect( page.locator( '.shqf-builder-palette' ) ).toBeVisible();
		// Blank form has no fields.
		await expect( page.locator( '.shqf-builder-canvas-empty' ) ).toBeVisible();
	} );
	...
	test( 'duplicate form creates copy', async ( { page } ) => {
		...
		const duplicateHref = await page.locator( '.wp-list-table tbody tr' ).first()
			.locator( '.row-actions a', { hasText: 'Duplicate' } ).getAttribute( 'href' );
		...
		await expect( page.locator( '.shqf-builder-title' ) ).toHaveValue( /Copy of/ );
	} );
} );
