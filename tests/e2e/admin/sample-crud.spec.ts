...
test.describe( 'Sample CRUD', () => {
	...
	test( 'sample appears in list via search', async ( { page } ) => {
		...
		await expect(
			page.locator( `.wp-list-table tr:has-text("${ name }")` )
		).toBeVisible();
	} );
	...
	test( 'sample lifecycle: archive, trash, restore, delete', async ( {
		page,
	} ) => {
		...
		await expect(
			page.locator( `tr:has-text("${ name }")` )
		).toBeVisible();

		// 2. Trash via row action.
		const trashHref = await page
			.locator(
				`tr:has-text("${ name }") .row-actions a:has-text("Trash")`
			)
			.getAttribute( 'href' );
		...
		const restoreHref = await page
			.locator(
				`tr:has-text("${ name }") .row-actions a:has-text("Restore")`
			)
			.getAttribute( 'href' );
		...
		const deleteHref = await page
			.locator(
				`tr:has-text("${ name }") .row-actions a:has-text("Delete Permanently")`
			)
			.getAttribute( 'href' );
		...
		// 7. Verify gone.
		await page.goto( SAMPLES_URL + '&status=trashed' );
		await expect( page.locator( `tr:has-text("${ name }")` ) ).toHaveCount(
			0
		);
	} );
	...
	test( 'category assignment on create', async ( { page } ) => {
		...
		await page.reload();
		await expect(
			page.locator( 'input[name="categories[]"]' ).first()
		).toBeChecked();
	} );
	...
	test( 'custom fields round-trip', async ( { page } ) => {
		...
		await page.reload();
		await expect(
			page.locator( 'input[name="cf_keys[]"]' ).first()
		).toHaveValue( 'Weight' );
		await expect(
			page.locator( 'input[name="cf_values[]"]' ).first()
		).toHaveValue( '250g' );
	} );
	...
	test( 'search finds sample by name', async ( { page } ) => {
		...
		await expect(
			page.locator( 'tr:has-text("Blue Fabric")' )
		).toBeVisible();
	} );
	...
	test( 'category filter works', async ( { page } ) => {
		...
		const catValue = await page
			.locator( '#filter-by-category option' )
			.nth( 1 )
			.getAttribute( 'value' );
