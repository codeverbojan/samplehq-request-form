...
test.describe( 'Settings page', () => {
	test( 'General tab saves privacy settings', async ( { page } ) => {
		...
		const retentionValue = await page.inputValue(
	...
	test( 'Spam tab saves Turnstile keys', async ( { page } ) => {
		...
		const siteKey = await page.inputValue(
			'input[name="shqf_turnstile_site_key"]'
		);
	...
	test( 'Email tab saves notification defaults', async ( { page } ) => {
		...
		await page.fill(
			'input[name="shqf_notification_email"]',
			'team@example.com'
		);
		...
		const fromName = await page.inputValue(
			'input[name="shqf_from_name"]'
		);
	...
	test( 'Connection tab shows disconnected state', async ( { page } ) => {
		await page.goto(
			'/wp-admin/admin.php?page=shqf-settings&tab=connection'
		);
